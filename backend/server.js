...
const connectDB = require("./config/db");
// const authRoutes = require("./routes/authRoutes");  // Comment out for now
// const adminRoutes = require("./routes/adminRoutes");
// const caregiverRoutes = require("./routes/caregiverRoutes");
// const timeClockRoutes = require("./routes/timeClockRoutes");  // ← CRITICAL
...
// ✅ ALL PUBLIC ROUTES (KEEP THESE - THEY WORK)
app.get("/", (req, res) => res.json({ message: "Caregiver Time Clock API ✅" }));
app.get("/api/ping", (req, res) => res.json({ ping: "ok" }));

// ✅ CRITICAL: Mock caregiver routes (PUBLIC)
app.get("/api/timeclock/mylogs", (req, res) => {  // ← mylogs NO HYPHEN
  res.json({
    logs: [
      { _id: "cg1-1", punchIn: "2025-12-28T10:00:00Z", punchOut: "2025-12-28T18:00:00Z" },
      { _id: "cg1-2", punchIn: "2025-12-28T09:00:00Z", punchOut: null }
    ]
  });
});

app.post("/api/timeclock/punch-in", (req, res) => {
  res.json({ message: "Clocked in successfully!", punchIn: new Date().toISOString() });
});

app.post("/api/timeclock/punch-out", (req, res) => {
  res.json({ message: "Clocked out successfully!", punchOut: new Date().toISOString() });
});
...
app.post("/api/auth/login", (req, res) => {
  ...
  if (user) {
    return res.json({
      message: "Login successful",
      token: `fake-token-for-${user.role}`,
      caregiver: { email: user.email, role: user.role }
    });
  }
  res.status(401).json({ message: "Invalid credentials" });
});

// ❌ COMMENT OUT ALL ROUTE MOUNTS
// app.use("/api/auth", authRoutes);
// app.use("/api/admin", adminRoutes);
// app.use("/api/caregivers", caregiverRoutes);
// app.use("/api/timeclock", timeClockRoutes);  // ← THIS WAS BLOCKING
